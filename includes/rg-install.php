...
$table_name = $wpdb->prefix.'rg_cities'; 
...
(
	...
	`rg_size` varchar(255) NOT NULL,
	`banner_type` enum('local','imported') NOT NULL DEFAULT 'local',
	`date` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	`status` enum('active','inactive') NOT NULL DEFAULT 'active',
	PRIMARY KEY (`rg_id`),
	UNIQUE KEY `rg_store_banner_id` (`rg_id`)
) $charset_collate;";
dbDelta($sql);

$table_name = $wpdb->prefix.'rg_ddeals'; 
$sql= "CREATE TABLE IF NOT EXISTS `$table_name` 
(
	`rg_id` int(11) NOT NULL AUTO_INCREMENT,
	`rg_deal_id` varchar(100) DEFAULT NULL,
	`rg_store_id` int(11),
	`title` varchar(255) NOT NULL,
	`description` text,
	...
	`status` enum('active','inactive') NOT NULL DEFAULT 'active',
	PRIMARY KEY (`rg_id`),
	UNIQUE KEY `rg_deal_id` (`rg_deal_id`)
) $charset_collate;";
dbDelta($sql);
?>